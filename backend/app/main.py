...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    if resolved_settings.trust_proxy_headers:
        app.add_middleware(
            ...
            trusted_hosts=resolved_settings.trusted_proxy_ips or '127.0.0.1',
        )

    if resolved_settings.trusted_host_patterns:
        app.add_middleware(
            TrustedHostMiddleware,
            allowed_hosts=resolved_settings.trusted_host_patterns,
            www_redirect=False,
        )

    app.add_middleware(
        CORSMiddleware,
        allow_origins=resolved_settings.resolved_cors_allow_origins() or ['*'],
